...
class home( View):
    ...
    def get(self, request):
        ...
        selected_services = random.sample(all_services, sample_size)  # Select 6 random services
...
class bookservice(LoginRequiredMixin, View):
    ...
    def post(self,request,id):
        ...
        messages.success(request, f"Your request for '{service_id.Name}' has been successfully submitted! We will contact you shortly.")
...
class admmin_home(LoginRequiredMixin, View):
    ...
    def get(self,request):
        ...
        # Calculate pending requests more accurately
        # A request is pending if it has a response that is not completed.
        assigned_requests_count = Response.objects.count()
        pending_requests = assigned_requests_count - completed_requests

        total_users = users.objects.count()
        
        # --- NEW: Fetch latest requests for the activity table ---
        ...
        context = {
            ...
            'latest_requests': latest_requests, # Pass new data to template
        }
...
class viewfeedbacks(LoginRequiredMixin, View):
    login_url = common_lib.DEFAULT_REDIRECT_PATH['ROOT']
    def get(self,request):
        feedback_records = Feedback.objects.select_related('User', 'Employ__admin').all().order_by('-Date')
...
class viewworkerfeedbacks(LoginRequiredMixin, View):
    ...
    def get(self,request):
        ...
        feedback_records = Feedback.objects.filter(Employ=current_worker).order_by('-Date')
        ...
        context= {
            'feedback_records': feedback_records,
            'average_rating': stats['average_rating'] or 0, # Default to 0 if no ratings
            'total_reviews': stats['total_reviews'] or 0,
        }
